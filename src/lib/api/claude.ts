...
### [2. 관상 & 인상 해석]  
- 눈, 입, 턱, 코의 흐름을 전체 인상과 연결해 해석  
- 단편적으로 "눈이 또렷하다"가 아니라, 그 인상이 주는 느낌을 중심으로  
- 실제 대화할 때 상대방이 느낄 수 있는 분위기까지 전달
...
`;
  ...
  private generateDummyResponse(request: ClaudeAnalysisRequest): ClaudeAnalysisResponse {
    return {
      ...
      faceReadingInterpretation: `당신의 눈을 보니, 마음을 읽는 능력이 뛰어난 분이라는 걸 알 수 있어요. 입술의 곡선은 따뜻한 말씨와 공감 능력을, 턱의 형태는 의지력과 리더십을 보여줍니다. 실제로 대화할 때 상대방은 "이 사람은 정말 내 마음을 이해해주는구나"라는 느낌을 받을 거예요. 전체적으로 균형 잡힌 인상이지만, 그 안에 숨어있는 따뜻함과 신뢰감이 가장 큰 매력 포인트입니다.`,
